...
## STATUS: COMPLETE (+ review iteration done)
...
### IMPLEMENTATION_ITERATION (post-c94cf9a review)
- S1 APPLIED: verify-release.sh now `gh attestation verify SHA256SUMS` before `sha256sum -c`;
  order = main.js attest → SHA256SUMS attest → checksums. Doc "Verifiable builds" synced.
- N1 ACCEPTED: release.sh runs `npm run build && npm test` before tagging (after clean-tree check).
- N2 REJECTED: fail-loud on duplicate remote tag is correct; no change.
- N3 REJECTED: repo H1 "… Plugin" is a descriptive title; manifest name already "Visit History".
- Verify: bash -n both scripts pass; shellcheck absent; scripts/docs only → build/lint/test unaffected.
- Still uncommitted for TOP_LEVEL_AGENT.
