...
Records visit history of notes, canvases, and Excalidraw drawings. Each completed focus session (start timestamp + duration) is appended to a per-user, per-document, per-device V3 log under `.plugin_data/visit_history/user/<user-name>/` (keyed by the file's persistent doc id, assigned on focus if missing; dot-hidden so Obsidian's file explorer/search never surface it — Obsidian Sync also skips dot-folders, so history is per-device and does NOT sync — accepted owner decision). Legacy v2 and `_visit_history/` (V1) data from older plugin versions is no longer read or written — content left untouched (owner decision); a pre-2026-07 `.visit_history/` dir is auto-renamed to the interim visible `__visit_history/`, its live `.vh_v3`+README files then moved under `.plugin_data/visit_history/`, and the pre-user-scoped `v2|v3` layout is auto-moved under `user/<user-name>/`. Still-visible legacy leftovers (`__visit_history/`, `_visit_history/`) are excluded from tracking/heatmap by `IsTrackedProvider`. Also provides a treemap heatmap visualization of vault file activity.
...
- `build.sh` — one-shot local dev bootstrap (npm install → `npm run dev`;
...
src/
  ...
  settings.ts              # Persisted settings: idleTimeoutSeconds (default 180, min 5),
                           # minFocusSecondsToRecord (default 2, min 0; 0 disables)
  ...
  core/
    ...
    focusDuration/
      ...
      MinDurationFilteringSink.ts # FocusDurationSink decorator: drops sessions
                                # shorter than the min-focus setting BEFORE the
                                # recorder (no trace at all); threshold live-read
    ...
    service/
      visitHistoryService/  # LastVisitProvider (read-only last-visit interface);
        ...
        user/               # VhUserPaths (.plugin_data/visit_history/user/<user-name>/ level),
      ...
      docId/                # DocIdBackfillService only (vault-wide backfill;
                            ...
                            # cross-plugin lock live in the stable-ids-for-obsidian
                            # npm package (its own repo)
      migration/            # VhTopDirRenameMigrationService (.visit_history →
                            # __visit_history; runs FIRST in onload) +
                            # VhPluginDataMoveMigrationService (__visit_history →
                            # .plugin_data/visit_history; runs SECOND, moves live
                            # .vh_v3+README only) + VhUserScopeMigrationService
                            # (pre-user-scoped v2/v3 → user/<user-name>/); cleanup
                            # rename+user-scope after 2026-October, move after 2027-February
...
- **VH V3 (focus durations)** is the only history read/written, under `.plugin_data/visit_history/user/<user>/v3/focus_duration_per_device/<device>/<doc-id>.vh_v3` — one completed session per line: `<ISO start stamp> D:<millis>`. The doc id IS the filename (survives renames; no backlink indirection). The top dir is deliberately dot-hidden under `.plugin_data/` — Obsidian's file explorer/search never surface it and plugins reacting to visible-folder creation are not disturbed; the trade-off (accepted) is that Obsidian Sync skips dot-folders so history is per-device and does NOT sync (a reversal of the earlier visible-`__visit_history` stance — see the issue URL at `VhUserPaths.TOP_DIR`). Because the Vault API never yields files under a dot-folder, the active dir needs no `IsTrackedProvider` gate; that gate excludes the still-visible legacy leftovers (`__visit_history/`, `_visit_history/`). ALL VH I/O goes through `HiddenFileUtil` (DataAdapter; reaches dot paths the Vault API can't). Ids that are not filename-safe (`DocIdFilenameSafety.isFilenameSafeId`) are skipped with `console.error`. `FocusDurationTracker` closes a session on navigation away (after a fixed 10 s unfocus grace — a same-doc refocus within grace continues the session so transient canvas-UI blips don't split it; the close is stamped at the ORIGINAL unfocus time, never inflating the duration), blur of the window HOSTING the doc, idle timeout (setting `idleTimeoutSeconds`, default 180 s, min 5 s, live-read — no reload needed; duration then ends at the LAST interaction — owner decision; also enforced retroactively so OS sleep is never counted; interaction after idle starts a NEW session), or unload flush (hard app quit can lose the last open session — accepted). A **minimum focus time** gate (`MinDurationFilteringSink`, setting `minFocusSecondsToRecord`, default 2 s, min 0, live-read; 0 disables) drops any session shorter than the threshold BEFORE the recorder, so a quick in-and-out visit leaves NO trace at all — no `.vh_v3` line AND no `LastVisitCache`/heatmap last-visit bump (owner decision; at the default, sub-2 s sessions incl. `D:0` pass-throughs are no longer recorded — a behavior change from older versions). Writes go through `VhV3DurationRecorder`'s single serialized chain.
- **User scoping** (`VhUserPaths`/`UserNameProvider`): all V3 data sits under `.plugin_data/visit_history/user/<user-name>/`, keeping histories of different people syncing one vault apart. The name is chosen by the human in a confirmation modal on `onLayoutReady` (unpinned devices only): pick an existing `user/<name>` dir (joining that identity) or type a new lowercase filename-safe name (`a-z0-9._-`, `UserNameSafety`; desktop pre-filled with the sanitized OS login name). Confirming pins it in device-scoped localStorage — FIRST PIN WINS, it never flips. Dismissing pins nothing: the plugin stays loaded (heatmap reads + doc-id assignment are name-independent) but NO VH is recorded until a name is pinned (re-prompted next start) — the V3 duration listener, user-scope migration, and README write all activate post-pin (`PluginFactory.activateUserScopedRecording`; activation REPLAYS the last focus event to the late-registered duration listener — serialized — so the doc already focused at startup/while the modal was up opens its session). Heatmap reads aggregate across ALL users (whole-vault activity — owner decision); writes go to the current user only. Two one-shot top-dir migrations run FIRST in `onload` — before the modal lists `.plugin_data/visit_history/user`: `VhTopDirRenameMigrationService` (`.visit_history/` → interim `__visit_history/`) then `VhPluginDataMoveMigrationService` (live `.vh_v3`+README `__visit_history/` → `.plugin_data/visit_history/`), each skipping + notifying on conflict; pre-user-scoped `v2|v3` dirs are then moved under the user by `VhUserScopeMigrationService` post-pin (all migrations never merge/delete; cleanup rename+user-scope after 2026-October, move after 2027-February).
- **Legacy VH data content is left untouched** (owner decision): v2 (wherever it sits — `.plugin_data/visit_history/user/<user>/v2/`, or stranded in the interim `__visit_history/v2/`) and `_visit_history/` (V1) are no longer read or written. The active dot-hidden `.plugin_data/visit_history/` needs no tracking gate; the still-visible legacy leftovers `__visit_history/` and `_visit_history/` are excluded from tracking/heatmap (`IsTrackedProvider`).
