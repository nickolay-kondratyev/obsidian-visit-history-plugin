...
const FILE_PATH = `.plugin_data/visit_history/user/${USER}/v3/focus_duration_per_device/${DEVICE}/${DOC_ID}.vh_v3`;
...
describe('VhV3DurationStore', () => {
  ...
  describe('getLastFocusStartMsAcrossUsersAndDevices', () => {
    ...
    const OTHER_DEVICE_FILE_PATH = `.plugin_data/visit_history/user/${USER}/v3/focus_duration_per_device/${OTHER_DEVICE}/${DOC_ID}.vh_v3`;
    const OTHER_USER_FILE_PATH = `.plugin_data/visit_history/user/bob/v3/focus_duration_per_device/bobs-host/${DOC_ID}.vh_v3`;
    ...
    it('should return null when devices exist but none tracked the doc', async () => {
      ...
      const { store, hidden } = setup();
      hidden.seedFile(
        `.plugin_data/visit_history/user/${USER}/v3/focus_duration_per_device/${DEVICE}/docid_OTHER_E.vh_v3`,
        '2026-07-09T22:02:15.745Z D:5600\n',
      );
      // WHEN looked up
    ...
    it('should return null for a user dir without any device dirs', async () => {
      ...
      const { store, hidden } = setup();
      hidden.seedFile(
        `.plugin_data/visit_history/user/${USER}/v3/README__generated__vh_v3_format.md`,
        'readme\n',
      );
      // WHEN looked up
      // THEN the doc was never focused
    ...
    it('should aggregate even when the doc is absent on one device', async () => {
      ...
      hidden.seedFile(FILE_PATH, '2026-07-09T10:00:00.000Z D:100\n');
      hidden.seedFile(
        `.plugin_data/visit_history/user/${USER}/v3/focus_duration_per_device/${OTHER_DEVICE}/docid_OTHER_E.vh_v3`,
