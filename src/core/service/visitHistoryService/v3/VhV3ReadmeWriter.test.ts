...
describe('VhV3ReadmeWriter', () => {
  describe('writeReadme', () => {
    it('should write the V3 format README at the documented path', async () => {
      // GIVEN a writer
      const hidden = new FakeHiddenFileUtil();
      const writer = new VhV3ReadmeWriter(hidden);
      // WHEN
      await writer.writeReadme();
      // THEN the README documents the V3 duration format
      expect(hidden.getContent(VhV3Paths.README_PATH)).toContain('D:<millis spent in focus>');
    });
  });
});
